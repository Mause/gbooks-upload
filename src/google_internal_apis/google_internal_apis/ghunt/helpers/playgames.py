...
async def get_player(
    ghunt_creds: GHuntCreds, as_client: httpx.AsyncClient, player_id: str
):
    ...
    with alive_bar(
        player_stats.played_games_count,
        title="🚎 Fetching played games...",
        receipt=False,
    ) as bar:
        _, next_page_token, played_games = await playgames.get_played_games(
            as_client, player_id
        )
        bar(len(played_games.games))
        while next_page_token:
            _, next_page_token, new_played_games = await playgames.get_played_games(
                as_client, player_id, next_page_token
            )
            played_games.games += new_played_games.games
            bar(len(new_played_games.games))

    with alive_bar(
        player_stats.achievements_count,
        title="🚎 Fetching achievements...",
        receipt=False,
    ) as bar:
        _, next_page_token, achievements = await playgames.get_achievements(
            as_client, player_id
        )
        bar(len(achievements.achievements))
        while next_page_token:
            _, next_page_token, new_achievements = await playgames.get_achievements(
                as_client, player_id, next_page_token
            )
...
async def search_player(
    ghunt_creds: GHuntCreds, as_client: httpx.AsyncClient, query: str
...
def output(player: Player):
    ...
    if player.played_games:
        ...
        if player.achievements:
            ...
            app_ids_count = dict(
                sorted(app_ids_count.items(), key=lambda item: item[1], reverse=True)
            )
            ...
            print(
                f"[+] Game with the most achievements : {target_game.game_data.name} ({achiv_nb})"
            )
