...
async def is_default_profile_pic(
    as_client: httpx.AsyncClient, image_url: str
) -> Tuple[bool, str]:
    """
    ...
    if (
        imagehash.hex_to_flathash(flathash, 8)
        - imagehash.hex_to_flathash("000018183c3c0000", 8)
        < 10
    ):
...
def show_version():
    ...
    print()
    gb.rc.print(
        f"> GHunt {current_version.metadata.get('version', '')} ({current_version.metadata.get('name', '')}) <".center(
            53
        ),
        style="bold",
    )
    ...
    if new_version:
        gb.rc.print(
            f"🥳 New version {new_metadata.get('version', '')} ({new_metadata.get('name', '')}) is available !",
            style="bold red",
        )
...
def check_new_version() -> tuple[bool, dict[str, str]]:
    ...
    """
    req = httpx.get(
        "https://raw.githubusercontent.com/mxrch/GHunt/master/ghunt/version.py"
    )
