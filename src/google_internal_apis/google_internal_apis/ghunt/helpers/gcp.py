...
async def project_nb_from_key(
    as_client: httpx.AsyncClient, ghunt_creds: GHuntCreds, api_key: str, fallback=True
) -> str | None:
    identitytoolkit_api = IdentityToolkitHttp(ghunt_creds)
    found, project_config = await identitytoolkit_api.get_project_config(
        as_client, api_key
    )
    if found:
    ...
    if fallback:
        ...
        req = await as_client.get(
            "https://blobcomments-pa.clients6.google.com/$discovery/rest",
            params={"key": api_key},
        )
        try:
