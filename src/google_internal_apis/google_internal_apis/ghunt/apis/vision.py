...
class VisionHttp(GAPI):
    def __init__(self, creds: GHuntCreds, headers: Dict[str, str] = {}):
        ...
        base_headers = {"X-Origin": "https://explorer.apis.google.com"}
    ...
    async def detect_faces(
        ...
        data_templates = {
            "default": {
                "requests": [
                    {
                        "features": [{"maxResults": 100, "type": "FACE_DETECTION"}],
                        "image": {},
                    }
                ]
            }
        ...
        if not data_templates.get(data_template):
            raise GHuntParamsTemplateError(
                f"The asked template {data_template} for the endpoint {endpoint.name} wasn't recognized by GHunt."
            )
        ...
        rate_limited = (
            req.status_code == 429
        )  # API Explorer sometimes rate-limit because they set their DefaultRequestsPerMinutePerProject to 1800
