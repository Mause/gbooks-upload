...
class CalendarHttp(GAPI):
    ...
    async def get_calendar(
        self, as_client: httpx.AsyncClient, calendar_id: str
    ) -> Tuple[bool, Calendar]:
    ...
    async def get_events(
        ...
        calendar_id: str,
        params_template="next_events",
        time_min=datetime.today().replace(tzinfo=timezone.utc).isoformat(),
        max_results=250,
        page_token="",
        ...
        if not params_templates.get(params_template):
            raise GHuntParamsTemplateError(
                f"The asked template {params_template} for the endpoint {endpoint.name} wasn't recognized by GHunt."
            )
