...
class GAPI(SmartObj):
    ...
    def _load_api(self, creds: GHuntCreds, headers: Dict[str, str]):
        ...
        if not is_headers_syntax_good(headers):
            raise GHuntCorruptedHeadersError(
                f"The provided headers when loading the endpoint seems corrupted, please check it : {headers}"
            )
    ...
    def _load_endpoint(self, endpoint: EndpointConfig):
        ...
        if key_name := endpoint.require_key:
            if not (api_key := get_api_key(key_name)):
                raise GHuntInsufficientCreds(
                    f"This endpoint requires the {key_name} API key in the GHuntCreds object, but it isn't loaded."
                )
            ...
            headers = {
                ...
                **headers,
                "Origin": endpoint.key_origin,
                "Referer": endpoint.key_origin,
            }
        ...
        if endpoint.authentication_mode == "sapisidhash":
            ...
            headers = {
                **headers,
                "Authorization": f"SAPISIDHASH {gen_sapisidhash(sapisidhash, endpoint.key_origin)}",
            }
        ...
        for ext_type, ext_value in endpoint.ext_metadata.items():
            ext_bin_headers = {
                f"X-Goog-Ext-{k}-{ext_type.title()}": v for k, v in ext_value.items()
            }
    ...
    async def _check_and_gen_authorization_token(
        self, as_client: httpx.AsyncClient, creds: GHuntCreds
    ):
        async with self.gen_token_lock:
            ...
            if self.api_name in creds.android.authorization_tokens:
                ...
                expiry_date = datetime.utcfromtimestamp(
                    creds.android.authorization_tokens[self.api_name]["expiry"]
    ...
    async def _query(
        ...
        if endpoint.verb == "GET":
            req = await as_client.get(
                f"{self.scheme}://{self.hostname}{base_url}",
                params=params,
                headers=headers,
                cookies=endpoint._computed_cookies,
            )
            ...
            if endpoint.data_type == "data":
                ...
                raise GHuntUnknownRequestDataTypeError(
                    f"The provided data type {endpoint.data_type} wasn't recognized by GHunt."
                )
        else:
            raise GHuntUnknownVerbError(
                f"The provided verb {endpoint.verb} wasn't recognized by GHunt."
            )
