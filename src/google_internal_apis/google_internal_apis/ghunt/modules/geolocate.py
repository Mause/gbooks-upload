...
async def main(
    as_client: httpx.AsyncClient, bssid: str, input_file: Path, json_file: Path = None
):
    # Verifying args
    body = None
    if input_file:
    ...
    location = geolocator.reverse(
        f"{resp.location.latitude}, {resp.location.longitude}", timeout=10
    )
    ...
    gb.rc.print(
        f"🗺️ [italic][link=https://www.google.com/maps/search/?q={resp.location.latitude},{resp.location.longitude}]Open in Google Maps[/link][/italic]\n",
